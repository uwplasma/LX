...
The script prints:
  ...
  - Flux checks on boundary and inner shell
  - Laplacian residual ∇²φ_s on an inner shell (FD approximation)

and produces plots:

  1. 3D: boundary colored by |B| and n·B/|B|.
  2. 1D: n·B/|B| on Γ vs index.
  3. 1D: ∇²φ_s at inner-shell sample points vs index.
...
def build_mfs_neumann_matrix(P, N, Y):
    ...
    Inputs:
      P : (N,3) boundary points x_i (world coordinates)
      N : (N,3) outward unit normals n_i
      Y : (M,3) MFS source points y_j on fictitious outer surface

    Returns:
      A : (N,M) dense JAX array
    """
    X = jnp.asarray(P)   # (N,3)
...
def solve_mfs_neumann(P, N, Y, g, reg=0.0):
    ...
    Inputs:
      P, N : boundary points and normals (N,3)
      Y    : MFS source points (M,3)
      g    : (N,) boundary data n·B_mv
      reg  : small Tikhonov regularization parameter (added to diag)

    Returns:
      c : (M,) MFS coefficients
    """
...
def make_mfs_single_layer_evaluators(Y_src, coeffs):
    ...
    Inputs:
      Y_src  : (M,3) source points y_j
      coeffs : (M,)  coefficients c_j

    Returns:
      phi_s_batch(X) : (K,) values of φ_s at K points
      grad_s_batch(X): (K,3) values of ∇φ_s at K points
    """
...
def solve_basis_fields_mfs(
    ...
    # Solve for basis 1: A c1 = -Dt in least-squares sense
    rhs1 = -At @ Dt
    rhs2 = -At @ Dp
    if reg != 0.0:
...
# ----------------------------------------------------------------------
# VMEC-like poloidal cross-section quadrature (unchanged logic)
# ----------------------------------------------------------------------
...
def main(xyz_csv,
    ...
    if mfs_out is None:
        mfs_out = xyz_csv.replace(".csv", "_mfs_energymin_vmec_like_solution.npz")
    ...
    try:
        ...
        print(f"[SAVE] Wrote MFS energy-minimizing solution checkpoint → {mfs_out}")
    except Exception as e:
