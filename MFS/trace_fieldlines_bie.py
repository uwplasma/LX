#!/usr/bin/env python3
"""
Trace field lines x'(t) = B(x) from a BIE vacuum solution checkpoint (.npz).

Here B = ∇φ is reconstructed from:
  φ(x) = φ_mv(x) + φ_s(x),

where
  - φ_s is a single-layer potential on the boundary with density σ:
        φ_s(x) = -∑_j σ_j W_j G(x, P_j),
        G(x,y) = 1/(4π|x-y|),
    evaluated with near-field regularization as in the BIE solver.

  - φ_mv is a multi-valued harmonic potential built from axis-aware
    toroidal/poloidal gradient bases (∇ϕ_a, θ̂) with coefficients a=(a_t,a_p)
    chosen by the energy-minimizing BIE code.

Usage
-----

  python trace_fieldlines_bie.py wout_precise_QH_bie_energymin_solution.npz \
      --nfp 4 --save-figure

  python trace_fieldlines_bie.py wout_precise_QA_bie_energymin_solution.npz \
      --nfp 2 --save-figure

Script is parallelized over multiple devices (set number_of_processors_to_use at top).

The .npz (produced by your BIE vacuum solver) must contain:
  center(3,), scale(scalar), P(N,3), N(N,3), W(N,),
  sigma(N,), a(2,), a_hat(3,), kind("torus"/"mirror"), phiedge(scalar)
"""
...
def _make_mv_grads(a_hat, P, N, center, scale):
    """
    Harmonic multivalued bases, consistent with the BIE solver:
    ...
    so any linear combo a_t grad_t + a_p grad_p is a vacuum field.
    """
    # We don't actually need P, N, center, scale here, but keep the
    # signature for compatibility with older code.
    a_hat = jnp.asarray(a_hat)
    a_unit = a_hat / jnp.maximum(1e-30, jnp.linalg.norm(a_hat))

    def grad_t(Xn):
        Xn = jnp.asarray(Xn).reshape((-1, 3))
        # broadcast constant axis vector to all points
...
def _make_single_layer_evaluators_bie(P_src, W_src, sigma, h_min, clip_factor=0.2):
    """
    Single-layer potential evaluators φ_s, ∇φ_s for the BIE density σ on boundary nodes P_src
    with area weights W_src, regularized with r_eff² = max(r², (clip_factor*h_min)²).
    """
    ...
    def phi_s_point(x):
        ...
        # NO minus here: match BIE solver
...
# ----------------------- Load BIE solution ------------------------- #
def load_bie_solution(npz_path: str):
    """
    Load BIE vacuum solution checkpoint and build evaluators:

      grad_point_fn(x): B(x) = ∇φ(x) = ∇φ_mv + ∇φ_s
      seeds_from_boundary(): inward-offset seeds along boundary

    The checkpoint must contain:
      center, scale, P, N, W, sigma, a, a_hat, kind, phiedge
    """
    d = np.load(npz_path, allow_pickle=False)

    center = jnp.asarray(d["center"], dtype=jnp.float64)   # (3,)
    scale_arr = d["scale"]
    scale = jnp.asarray(scale_arr.item() if scale_arr.shape == () else float(scale_arr),
                        dtype=jnp.float64)
    ...
    sigma = jnp.asarray(d["sigma"], dtype=jnp.float64)     # (N,)
    a = jnp.asarray(d["a"], dtype=jnp.float64)             # (2,)
    a_hat = jnp.asarray(d["a_hat"], dtype=jnp.float64)     # (3,)
    kind = str(d["kind"])
    # phiedge = float(d["phiedge"])   # not needed for tracing, but present

    # Estimate a spacing from the stored area weights: W ≈ π h^2 => h ≈ sqrt(W/π)
    h_eff = jnp.sqrt(W / jnp.pi)
    h_min = float(jnp.min(h_eff))

    # Multivalued bases (normalized coordinates)
    grad_t, grad_p = _make_mv_grads(a_hat, P, N, center, scale)

    # Single-layer potential evaluators
    phi_s_point, grad_s_point = _make_single_layer_evaluators_bie(
        P_src=P, W_src=W, sigma=sigma, h_min=h_min, clip_factor=0.2
    )
    ...
    # Total field B = ∇φ = ∇φ_mv + ∇φ_s
    @jax.jit
    def grad_point_fn(x: jnp.ndarray) -> jnp.ndarray:
        return grad_mv_point_world(x) + grad_s_point(x)

    # Optional scalar potential (for diagnostics); just store φ_s for now
    @jax.jit
...
def seeds_along_axis_from_boundary(
    ...
    # Slight tweak to avoid being exactly on boundary
    pL = (pL + pR) / 2.01
    pR = pR * 0.99
...
def _boundary_field_with_jump(
    ...
    """
    B on Γ (boundary) using the same jump relation + singularity subtraction
    as in the BIE solver:

      - B_mv from harmonic multivalued bases (constant + azimuth).
      - Tangential part of B_s from singularity subtraction:
          B_s^tan = -∑_j (σ_j - σ_i)W_j ∇G(x_i,x_j)  (projected to tangent)
      - Normal part of B_s from jump formula:
          n·B_s = -½ σ + K'σ
          B_s^n = (n·B_s) n_hat

      B_tot = B_mv + B_s^tan + B_s^n at the boundary nodes P.
    """
    Pj = jnp.asarray(P, dtype=jnp.float64)   # (N,3)
    Nj = jnp.asarray(N, dtype=jnp.float64)   # (N,3)
    Wj = jnp.asarray(W, dtype=jnp.float64)   # (N,)
    sj = jnp.asarray(sigma, dtype=jnp.float64)  # (N,)
    a_vec = jnp.asarray(a, dtype=jnp.float64)   # (2,)
    ...
    # Estimate local spacings from W (same as in solver: W ~ π h²).
    h = jnp.sqrt(Wj / jnp.pi)
    h_min = jnp.min(h)
    r2_clip = (clip_factor * h_min) ** 2

    # Normalized coordinates for MV bases
    Xn = (Pj - center_j[None, :]) * scale_j

    # Harmonic bases: grad_t = a_hat (constant), grad_p = ∇ϕ_a
    a_unit = a_hat_j / jnp.maximum(1e-30, jnp.linalg.norm(a_hat_j))
    Gt = jnp.broadcast_to(a_unit, Xn.shape)       # (N,3)
    Gp = _grad_azimuth_about_axis(Xn, a_hat_j)    # (N,3)

    B_mv = scale_j * (a_vec[0] * Gt + a_vec[1] * Gp)   # (N,3)

    # Pairwise geometry on Γ
    Xi = Pj[:, None, :]      # (N,1,3)
    Xj = Pj[None, :, :]      # (1,N,3)
    diff = Xi - Xj           # (N,N,3)
    ...
    # K' for normal derivative (for jump-normal part)
    n_hat = Nj / jnp.maximum(1e-30, jnp.linalg.norm(Nj, axis=1, keepdims=True))
    n_dot_grad = jnp.sum(gradG * n_hat[:, None, :], axis=-1)       # (N,N)
    n_dot_grad = jnp.where(mask, n_dot_grad, 0.0)
    Kprime = n_dot_grad * Wj[None, :]                              # (N,N)

    # Tangential B_s via singularity subtraction
    Sigma_j = sj[None, :]                 # (1,N)
    Sigma_i = sj[:, None]                 # (N,1)
    weight = (Sigma_j - Sigma_i) * Wj[None, :]   # (N,N)

    Bs = -jnp.einsum("ij,ijk->ik", weight, gradG)   # (N,3)

    n_dot_Bs = jnp.sum(n_hat * Bs, axis=1, keepdims=True)
    Bs_tan = Bs - n_hat * n_dot_Bs                  # tangential component

    # Normal B_s from jump relation: n·B_s = -½σ + K'σ
    Ksigma = Kprime @ sj                            # (N,)
    n_dot_B_s = -0.5 * sj + Ksigma                  # (N,)
    B_s_n = n_hat * n_dot_B_s[:, None]              # (N,3)

    B_s = Bs_tan + B_s_n
    B_tot = B_mv + B_s

    return np.asarray(B_tot)
...
def main(
    ...
    # Load BIE checkpoint & evaluators
    m = load_bie_solution(mfs_npz)
    ...
    print(f"[BOX] Integration box: x[{x_min:.3f}, {x_max:.3f}], "
    ...
    # --- Boundary-aware |B| on Γ using jump + singularity subtraction ---
    B_bdry = _boundary_field_with_jump(
        P=P,
        N=N,
        W=m["W"],
        sigma=m["sigma"],
    ...
    Gm = np.linalg.norm(B_bdry, axis=1)   # |B| on boundary
    ...
    ax_3d.set_ylabel("y")
    ax_3d.set_zlabel("z")
    ax_3d.set_title("Field lines of B = ∇φ (BIE)")
...
if __name__ == "__main__":
    # Point this at your BIE solution NPZ
    # Examples (adjust to your filenames):
    default_solution = "wout_precise_QA_bie_energymin_vmec_like_solution.npz"
    # default_solution = "wout_precise_QH_bie_energymin_vmec_like_solution.npz"
    # default_solution = "wout_SLAM_4_coils_bie_energymin_vmec_like_solution.npz"
    # default_solution = "knot_tube_bie_energymin_vmec_like_solution.npz"
    ...
    ap.add_argument("file", nargs="?", type=str,
                    ...
                    help="Path to BIE solution .npz (positional or --file).")
    ap.add_argument("-f", "--file", dest="file", type=str,
                    help="Path to BIE solution .npz (overrides positional if both given).")
    ...
    ap.add_argument("--save-figure", action="store_true", default=True,
                    help="Save figures to disk instead of just showing.")
    ...
    ap.add_argument("--seed-mode", choices=["axis", "boundary"], default="axis",
                    help="axis: chord across center using a_hat; "
                         "boundary: old inward-offset sampling")
    ...
    ap.add_argument("--seeds", type=str, default=seeds_default,
                    help="Comma-separated list of seed points as x:y:z,x:y:z,... "
                         "(overrides automatic seed computation)")
