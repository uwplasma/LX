...
This version:
  ...
  * Uses Diffrax to find the magnetic axis (for toroidal / mirror geometries).
  * For kind=="knot", uses a Cartesian grid and builds an "axis band"
    from the deepest interior nodes instead of tracing a toroidal axis.
...
def plot_3d_axis_boundary_interp(P, axis_pts, psi3, xs, ys, zs):
    ...
    ax.scatter(
        ...
        s=0.3, alpha=0.9, edgecolor="k", label="magnetic axis band",
    )
...
###############################################################################
# Axis finding via Poincaré map with Diffrax (torus / mirror)
###############################################################################
...
###############################################################################
# FCI connectivity (Euclidean and cylindrical)
###############################################################################
...
# ... (FCI connectivity builders and trilinear weights unchanged; omitted here
# in comment for brevity, but they are identical to your original script) ...

# === KEEP ALL YOUR EXISTING FCI / OPERATOR CODE HERE UNCHANGED ===
# (build_fci_connectivity_chunked, build_fci_connectivity_cylindrical,
#  _find_cell_indices, trilinear_weights,
#  make_fci_operator_jax, make_fci_operator_cylindrical,
#  make_linear_operator_jax, cg_jax)
# I’m leaving them exactly as in your message.

# ------------------------------------------------------------------
# For completeness I'll paste them too, unchanged.
# ------------------------------------------------------------------
...
def build_fci_connectivity_chunked(
    ...
    if dphi_per_step is None:
        N_par = fci_planes_per_field_period
    ...
    for chunk_start in range(0, n_inside_total, chunk_size):
        ...
        B_chunk = np.asarray(_grad_phi_batched(X_chunk_j))
...
def build_fci_connectivity_cylindrical(
    ...
    phi_period = phi_max - phi_min + (phis[1] - phis[0])
...
def solve_fci(npz_file: str, grid_N: int = 64, N_phi: int = 128, eps: float = 1e-3, band_h: float = 1.5,
    ...
    # Robustly read the geometry kind written by main.py
    kind_raw = data.get("kind", "torus")
    kind_str = str(kind_raw).strip().lower()

    # --- OVERRIDE: treat knot_tube solutions as "knot" ---
    base_name = os.path.basename(npz_file).lower()
    if "knot" in base_name:
        print(f"[INFO] Overriding kind='{kind_str}' to 'knot' based on filename '{base_name}'.")
        kind_str = "knot"

    is_torus  = (kind_str == "torus")
    is_knot   = (kind_str == "knot")
    is_mirror = (kind_str == "mirror")

    if verbose:
        pinfo(
            f"Loaded checkpoint with {P.shape[0]} boundary points and "
            f"{Yn.shape[0]} multipole sources (kind_raw={kind_raw}, kind={kind_str})."
        )
    ...
    # ------------------------------------------------------------------
    # Build grid: cylindrical (R, φ, Z) when kind=torus; Cartesian otherwise.
    # "knot" and "mirror" are handled with a Cartesian grid.
    # ------------------------------------------------------------------
    if is_torus:
        Rb = np.sqrt(P[:, 0]**2 + P[:, 1]**2)
        Zb = P[:, 2]
        ...
        RR, PHI, ZZ_cyl = np.meshgrid(Rs, phis, Zs, indexing="ij")
        XX = RR * np.cos(PHI)
        YY = RR * np.sin(PHI)
        Xq = np.column_stack([
            XX.ravel(order="C"),
        ...
        # Cartesian for mirror / knot / generic
    ...
    # Characteristic boundary band thickness
    h_band_vox = float(band_h) * voxel

    # ------------------------------------------------------------------
    # Axis & axis band
    #   - non-knot (torus / mirror): use toroidal Poincaré + Diffrax
    #   - knot: build axis band from deepest interior nodes
    # ------------------------------------------------------------------
    if not is_knot:
        # ---- Old axis finder (torus / mirror-like) ----
        phi_tol = 0.2
        ...
        # --- Check if axis is obviously wrong (far from surface or all outside) ---
        axis_all_outside = (inside_axis.sum() == 0)
        # Compare distance to typical device size
        geom_size = np.linalg.norm(P.max(axis=0) - P.min(axis=0))
        too_far = (np.abs(signed_axis.min()) > 2.0 * geom_size)

        if axis_all_outside or too_far:
            pinfo("[WARN] Axis-finding appears to have failed (axis outside domain).")
            pinfo("[WARN] Falling back to 'deepest interior' axis band construction (knot-like).")

            # Rebuild axis band exactly like the KNOT branch:
            inside_idx = np.where(inside_mask)[0]
            if inside_idx.size == 0:
                raise RuntimeError("Inside mask empty when building fallback axis band.")

            depth_inside = -signed_dist[inside_idx]
            depth_inside = np.maximum(depth_inside, 0.0)
            max_depth = depth_inside.max()
            if max_depth <= 0.0:
                raise RuntimeError("[FALLBACK] Could not identify interior depth; check geometry.")

            frac = 0.03
            n_axis_nodes = max(32, int(frac * inside_idx.size))
            order_desc = np.argsort(-depth_inside)
            chosen = inside_idx[order_desc[:n_axis_nodes]]

            axis_band = np.zeros_like(inside_mask, dtype=bool)
            axis_band[chosen] = True

            axis_band_radius_eff = float(depth_inside[order_desc[:n_axis_nodes]].mean())
            axis_pts = Xq[axis_band]

            if verbose:
                pinfo(f"[FALLBACK] Axis band nodes: {axis_band.sum()} / {inside_mask.sum()}, "
            ...
            if not np.any(axis_band):
                ...
                d_axis_all, _ = nbrs_axis.kneighbors(Xq)
                d_axis_all = d_axis_all[:, 0]
                ...
                order = np.argsort(d_axis_all[inside_idx])
                chosen = inside_idx[order[:n_axis_nodes]]

                axis_band = np.zeros_like(inside_mask, dtype=bool)
                axis_band[chosen] = True
                axis_band_radius_eff = float(d_axis_all[chosen].max())
        ...
        # ------------------------------------------------------------------
        # ### KNOT: axis band based on maximal distance from boundary
        # ------------------------------------------------------------------
        if verbose:
            pinfo("[KNOT] Building axis band from deepest interior nodes (no toroidal axis solve).")

        inside_idx = np.where(inside_mask)[0]
        if inside_idx.size == 0:
            raise RuntimeError("Inside mask empty when building knot axis band.")
        ...
        max_depth = depth_inside.max()
        if max_depth <= 0.0:
            raise RuntimeError("[KNOT] Could not identify interior depth; check geometry.")

        # Take the deepest few percent of nodes as "axis band"
        frac = 0.03   # 3% deepest nodes
        n_axis_nodes = max(32, int(frac * inside_idx.size))
        order_desc = np.argsort(-depth_inside)   # largest depth first
        chosen = inside_idx[order_desc[:n_axis_nodes]]
        ...
        if axis_pts.shape[0] == 0:
            # extreme fallback: take single deepest point
            idx0 = inside_idx[np.argmax(depth_inside)]
            axis_band[idx0] = True
            axis_pts = Xq[idx0:idx0+1]
            axis_band_radius_eff = float(depth_inside.max())

        # Define "mean" cylindrical coordinates for diagnostics
        R_axis = float(np.mean(np.sqrt(axis_pts[:, 0]**2 + axis_pts[:, 1]**2)))
        Z_axis = float(np.mean(axis_pts[:, 2]))

        if verbose:
            pinfo(f"[KNOT] Axis band nodes: {axis_band.sum()} / {inside_mask.sum()}, "
                  f"effective 'radius' ≈ {axis_band_radius_eff:.3e}")
            pinfo(f"[KNOT] Approximate mean (R,Z) for axis: ({R_axis:.3e}, {Z_axis:.3e})")

        inside_axis, _, signed_axis = inside_mask_from_surface(P, Nsurf, axis_pts)
        if verbose:
            pinfo(
                f"[KNOT] Axis-band points vs surface signed distance: "
                f"min={signed_axis.min():.3e}, max={signed_axis.max():.3e}"
            )
            pinfo(f"[KNOT] Axis-band points classified inside: {inside_axis.sum()} / {inside_axis.size}")

    # ------------------------------------------------------------------
    # Boundary band (outer ribbon) and clash resolution
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Evaluate ∇φ on inside nodes only
    # ------------------------------------------------------------------
    if verbose:
    ...
    G_inside = np.asarray(grad_phi(X_inside_j))
    G = np.zeros_like(Xq)
    ...
    # ------------------------------------------------------------------
    # Build PDE operator (FCI or tensor)
    # ------------------------------------------------------------------
    if use_fci:
        if verbose:
            pinfo("Building FCI connectivity (field-line mapping) ...")

        if grid_is_cyl and is_torus:
            Rs = grid_axes["Rs"]
            phis = grid_axes["phis"]
            Zs = grid_axes["Zs"]
        ...
        dy = grid_axes["dy"]
        dz = grid_axes["dz"]
        D = diffusion_tensor_jax(G, eps=eps, delta=delta)
    ...
    # ------------------------------------------------------------------
    # Alignment metric q on interior core
    # ------------------------------------------------------------------
    ...
    core_metric = (
        ...
        & (~band3[1:-1, 1:-1, 1:-1])
        & (~axis3[1:-1, 1:-1, 1:-1])
        & (fci_valid3[1:-1, 1:-1, 1:-1] if use_fci else True)
    )
    ...
    grad_floor = 1e-3 * np.nanmax(grad_mag_full)
    metric_mask = (
        core_metric
        & (gnorm_core > 1e-10)
    ...
    psi_inside = psi[inside_mask & (~band) & (~axis_band)]
    if psi_inside.size > 0:
        unique_vals = np.unique(np.round(psi_inside, decimals=6))
        if unique_vals.size == 1:
            pinfo(f"[WARN] ψ is effectively constant in interior (value ≈ {unique_vals[0]:.3f}). "
                  "This usually indicates a degenerate BC / axis-band issue.")
    ...
    if plot:
        ...
        try:
            if grid_is_cyl and kind_str == "torus":
                ...
                R_axis_arr   = np.sqrt(axis_pts[:, 0]**2 + axis_pts[:, 1]**2)
                phi_axis_arr = np.mod(np.arctan2(axis_pts[:, 1], axis_pts[:, 0]), 2*np.pi)
                Z_axis_arr   = axis_pts[:, 2]
                ...
                figRZ = plot_psi_maps_RZ_panels(
                    ...
                    R_axis=R_axis_arr, Z_axis=Z_axis_arr, phi_axis=phi_axis_arr,
                ...
                power = psi_power_for_plot
                figRZ = plot_psi_maps_RZ_panels(
                    np.power(psi_RZphi, power), Rs_cyl, phis_cyl, Zs_cyl, jj_list,
                    Rb=Rb, Zb=Zb, phi_b=phi_b,
                    R_axis=R_axis_arr, Z_axis=Z_axis_arr, phi_axis=phi_axis_arr,
                    title=rf"$\psi(R,Z)^{power}$"
                )
        ...
        try:
            band3 = band.reshape(nx, ny, nz)
            P_bnd_grid = np.column_stack(np.nonzero(band3))
...
if __name__ == "__main__":
    ...
    parser.add_argument("npz", nargs="?", default=resolve_npz_file_location(default_solution),
                        help="MFS solution checkpoint (*.npz) containing center, scale, Yn, alpha, a, a_hat, P, N, kind")
